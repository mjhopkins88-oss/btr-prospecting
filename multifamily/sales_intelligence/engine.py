...
def build_sales_intelligence(
    ...
    if log and not lead.is_demo:
        _maybe_log_event(lead.id, context, strategy, reasoning, variant, guardrail_status)
...
def _decision_tuple(context, strategy):
    return (
        ...
        strategy.recommended_action, strategy.conversation_mode,
    )


def _maybe_log_event(lead_id: str, context, strategy, reasoning, variant: int, guardrail_status: str) -> None:
    ...
    if latest and variant == 0:
        prev_tuple = (
            ...
            latest.get('recommended_action'), latest.get('conversation_mode'),
        )
